...
func (enc *BitwisePublicEncryptor) Encrypt(value bool) *Ciphertext {
	...
	if err := enc.enc.Encrypt(pt, ct); err != nil {
		panic(fmt.Errorf("public key encrypt: %w", err))
	}
	ct.IsNTT = true
...
func (enc *BitwisePublicEncryptor) EncryptUint64(value uint64, t FheUintType) *BitCiphertext {
	numBits := t.NumBits()
	bits := make([]*Ciphertext, numBits)

	for i := 0; i < numBits; i++ {
		bit := (value >> i) & 1
		bits[i] = enc.Encrypt(bit == 1)
	}
...
func NewBitwiseEvaluator(params Parameters, bsk *BootstrapKey, sk *SecretKey) *BitwiseEvaluator {
	return &BitwiseEvaluator{
		params: params,
		eval:   NewEvaluator(params, bsk, sk),
	}
}
